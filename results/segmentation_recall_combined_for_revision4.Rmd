...
L.BAD.SUBJ.CPUTIME <- list (
    tstbl = list (
        c(subj = "399612_200413_124119_2163c2ed5ac2dd37063193b689dafe82251f433e.csv",
          response = "dalonigtbdophophi dalobdakabdarobigopachu"),
        
        c(subj = "399612_210517_101428_d6832877cd6a16ecb1498f99ff25a2ee66096d93.csv",
        ...
        c(subj = "399612_210517_100654_b3a2d858e648fab540f26d63cd60cdf40be0ad25.csv",
          response = "takahsakakakaratatataikokokokotatakatakatakatakatakatakataka"),
        
        c(subj = " 399612_210517_101201_0e335a2e6bcd07eaf32c06cd9a1a7c2e794601ee.csv",
          response = "dabroobitalooki,bkuti2,golab"),
        
        c(subj = "399612_210524_062929_10c3df303f8a5b47751465793bab45d638f079f4.csv",
          response = "matikulatatitulapapitularimatitulaatitula"),
        
        c(subj = "399612_210524_115828_a59856877975c71a1a7b9e9e3f776cb992aaebde.csv",
          response = "tu kalla ti palla tuti kulla papi pu tu kalla ti palla tuti kulla papi pu"),
        
        c(subj = "399612_210524_120014_a4d641ab312e71e300bd349948e4fcc7e936105e.csv",
          response = "tutopitulakatutopitoolaka"),
        
        c(subj = "399612_210524_120523_06795ef32f11db08be1b1336d62b8682d9f71cc5.csv",
          response = "papikuchi,butalapapikuchi,kukala,pikala,budharapikuchi,chupapikachubudarapi"),
        
        c(subj = "399612_210602_064236_2c0cec9dcb1be2a6bddff35a85c2e5d558fac9eb.csv",
          response = "da putty da raboo,da puppy da raboo,da raboo,da raboo,da puppy da rabooo"),
        
        c(subj = "399612_210602_064353_5daaf2d0bb79fd346179a286ad5f47980fb63672.csv",
          response = "rabi tiku ko kolada fabi ,rabi tiku ko la dafabi...."),
        
        c(subj = "399612_210602_072517_d3db595db75dedf7869e0e6a4a0e39c6541b361e.csv",
...
prepare.data.for.streamType.contrast <- function (dat = ., value.var, mu = 0) {
    ...
    dat %>%     
        ...
        dplyr::summarize(
            Continuous = wilcox.p(continuous, mu, add.descriptives = TRUE, print.n = TRUE),
            Segmented = wilcox.p(segmented, mu, add.descriptives = TRUE, print.n = TRUE),
...
# The particpants removed due to CPU time problems are not excluded here, but this data
# frame is just used to be left join to the recall data, where subjects are removed

if (ANALYZED.DATA.SETS["TESTABLE"]){
    dat.recall.tstbl.recognition.m <- dat.recall.tstbl %>% 
        dplyr::filter(myPhase == "test_recognition") %>%
        dplyr::group_by(filename, lang, mySegmentationCond) %>%
        dplyr::summarize(N = n(),
...
dat.recall.combined.m.p.correct.initial.syll <- dat.recall.combined.m %>% 
    prepare.data.for.streamType.contrast("p.correct.initial.syll", c(4/12, .75/2))
...
dat.recall.combined.m.p.correct.final.syll <- dat.recall.combined.m %>% 
    prepare.data.for.streamType.contrast("p.correct.final.syll", c(4/12, .75/2))
# Changed 03/19/2022
...
dat.recall.combined.m.all.results.main <-
    bind_rows(
        dat.recall.combined.m %>% 
            ...
            dplyr::mutate (filter = "Recognition accuracy"),
        
        dat.recall.combined.m.n.items %>% 
            dplyr::mutate (filter = "Number of items"),
        dat.recall.combined.m.n.syll %>% 
            dplyr::mutate (filter = "Number of syllables/item"),
        ...
        dat.recall.combined.m.p.words.part.words %>% 
            dplyr::mutate (filter = "Proportion of words among words and part-words (or concatenations thereof)" ),
        
        dat.recall.combined.m.forward.tps %>% 
            dplyr::mutate (filter = "Forward TPs"),
        ...
        dat.recall.combined.m.backward.tps %>% 
            dplyr::mutate (filter = "Backward TPs"),
        ...
        dat.recall.combined.m.p.high.tp.chunk.low.tp.chunk %>% 
            dplyr::mutate (filter = "Proportion of High-TP chunks among High- and Low-TP chunks"),
        
        
        dat.recall.combined.m.p.correct.initial.syll %>% 
            dplyr::mutate (filter = "Proportion of items with correct initial syllables"),
        dat.recall.combined.m.p.correct.final.syll %>% 
            dplyr::mutate (filter = "Proportion of items with correct final syllables")
...
```{r recall-w-pw-chunks-positions-plot-positions, fig.cap="Analyses of the participants' productions. (a) Proportion of words among words and part-words. The dotted line represents the chance level of 50 percent in a two-alternative forced-choice task, while the dashed line represents the chance level of 33 percent that an attested 3 syllable-chunk is a word rather than a part-word. (b) Proportion of high-TP chunks among high- and low-TP chunks. The dashed line represents the chance level of 66 percent that an attested 2 syllable-chunk is a high-TP rather than a low-TP chunk. (c) proportion of productions with correct initial syllables and (d) with correct final syllables. The dotted line and the dashed line represent alternative the chance levels of 33 and 37.5 percent, respectively."}
ggpubr::ggarrange (
    plot.recall.combined.m.p.correct.initial.syll + 
            geom_hline(yintercept = .75/2, lty = 5),
    plot.recall.combined.m.p.correct.final.syll +
        geom_hline(yintercept = .75/2, lty = 5),
    nrow = 1, ncol = 2,
    #common.legend = TRUE, legend="bottom", 
...
#' Create Ordered Label Mappings
#'
#' This function generates an ordered mapping of full names to either shorthand labels 
#' or vice versa, depending on the direction specified ("to_order" or "to_label"). See examples below
#' 
#'
#' @param full_names A vector of corresponding full names for the labels.
#' @param direction A string indicating the direction of the mapping. Either:
#'   \itemize{
#'     \item "to_order" (default): Maps full names to shorthand labels and orders them by the length of the input vector.
#'     \item "to_label": Maps shorthand labels to full names (reverses the order).
#'   }
#' @return A named vector that either maps full names to labels or labels to full names,
#'   with the ordering preserved.
#' @examples
#' full_names <- c("average_fw_tp", "p.correct.initial.syll", "p.correct.final.syll", "correct_segm")
#' create_ordered_mapping(full_names, direction = "to_order")
#' #          average_fw_tp p.correct.initial.syll   p.correct.final.syll           correct_segm 
#'              "a"                    "b"                    "c"                    "d" 
#' create_ordered_mapping(full_names, direction = "to_label")
#' #               a                        b                        c              d
#' #         "average_fw_tp" "p.correct.initial.syll"   "p.correct.final.syll"     "correct_segm" 
#' 
#' @export
create_ordered_mapping <- function(x = ., direction = "to_order") {
    
  # Generate shorthand labels based on the length of full_names
  labels <- letters[1:length(x)]  # Uses letters a, b, c, d, etc.

  # Create an ordered mapping from labels to full names given in x
  label_mapping <- labels %>% 
    setNames(x) 

  # If direction is "to_order", return the mapping from full names to labels
  if (direction == "to_order") {
    return(label_mapping)
  }
  
  # If direction is "to_label", reverse the mapping (from labels to full names)
  if (direction == "to_label") {
    return(setNames(names(x), x))
  }

  # Handle invalid direction argument
  stop("Direction must be either 'to_order' or 'to_label'.")
}


#' Discard List Element by Name
#'
#' This function removes elements from a list based on their names. It uses the `purrr::discard`
#' function to discard any element where the name matches a given pattern (using `str_detect`).
#'
#' @param l A list from which elements will be removed.
#' @param name_to_be_removed A string or regular expression pattern that matches the names 
#'                           of elements to be removed from the list.
#' 
#' @return A list with the elements matching the name or pattern removed.
#' 
#' @examples
#' # Example list
#' my_list <- list(a = 1, b = 2, c = 3, d = 4)
#' 
#' # Discard element with name "b"
#' discard_list_element(my_list, "b")
#' # Expected output: list(a = 1, c = 3, d = 4)
#' 
#' # Discard element whose name contains "c"
#' discard_list_element(my_list, "c")
#' # Expected output: list(a = 1, b = 2, d = 4)
#' 
#' @export
discard_list_element <- function(l = ., name_to_be_removed) {
    
    l %>% 
        purrr::discard(str_detect(names(.), name_to_be_removed))
}

# Create a named vector to map variable names to shorthand labels (letters a-d).
# This helps with ordering and labeling in plots and data transformations.
v.correlations.label.order <- create_ordered_mapping(c("average_fw_tp",
                "p.correct.initial.syll",
                "p.correct.final.syll",
               "correct_segm"),
               direction = "to_order")
# The reverse is
# setNames (names (v.correlations.label.order), v.correlations.label.order)
...
v.correlations.labels.print <- c(
    ...
    setNames (., v.correlations.label.order[names(.)]) # set names to a, b, c ...


    
dat.recall.combined.m.correlations <- dat.recall.combined.m %>% 
    dplyr::select(data.set, streamType,
                  create_ordered_mapping(v.correlations.label.order, direction = "to_label") %>% unname)  %>% 
    dplyr::group_by(data.set, streamType) %>% 
    dplyr::group_modify (~ .x %>% 
                             as.matrix %>% 
                             Hmisc::rcorr (., type = "spearman") %>% 
                             as.list %>% 
                             discard_list_element("type")%>% 
...
```{r snapshot-save, echo=TRUE, eval = FALSE}
# library(renv)
# renv::snapshot()
```
