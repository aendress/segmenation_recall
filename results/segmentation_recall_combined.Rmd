...
wilcox.p <- function (x, mu = 0, add.descriptives = FALSE)
{
    x <- as.numeric (x)
    ...
    if (add.descriptives){
        ...
        res <- paste0(
            "\\M = ", signif (mean (x, na.rm = TRUE), getOption('digits')),
            ", \\SE = ", signif (se (x), getOption('digits')),
            ", \\p = ", res
        )
    }
...
dat.recognition.city <- rbind(read.table ("data/city/recall.i.e3.cont.tab", 
                               header=T, sep="\t", comment.char = "%"),
                     read.table ("data/city/recall.i.e4.segm.tab",
                                 header=T, sep="\t", comment.char = "%")) %>% 
    ...
    dat.recall.city <- gdata::read.xls(
        "data/city/segmentation_recall_transcriptions.xlsx", 
                                       sheet="Sheet2-ade",
                                       stringsAsFactors = FALSE,
                                       header=TRUE) %>%
...
if (ANALYZED.DATA.SETS["TESTABLE"]){
    # Data from testable
    dat.recall.tstbl <-
        read.testable.results("data/testable/399612_results_20200424_0326", 
                              comment.char = "",
                              stringsAsFactors = FALSE)  %>%
...
dat.recognition.combined.m %>% 
    ...
    kable (caption = "\\label{tab:recognition_descriptives}Descriptives for the recognition test", booktabs = TRUE, escape = FALSE) %>%
    #     kable_styling() %>%
...
The substitution rules employed in the current experiment are shown in Table \ref{tab:substitution_rules}.
...
dat.recall.demographics.combined %>%
    ...
    knitr::kable(caption = '\\label{tab:recall_demographics}Demographics of the final sample. Note that the City participants completed both segmentation conditions.', booktabs = TRUE, escape = FALSE) %>%
...
## General measures
We first calculate the number of items produced as well their average, and compare them  against the zero as well as across segmentation conditions. As shown in Table \ref{tab:recall-general-results-print} and Figure \ref{fig:recall-general-measures-plot}, participants produced positive number of items. Neither the number of items produced nor their number of syllables differed across the segmentation conditions.
...
```{r recall-general-measures-plot, fig.cap="Number of items produced as well as their numbers of syllables."}
ggpubr::ggarrange (
...
As shown in Table \ref{tab:recall-general-results-print} and Figure \ref{fig:recall-positions-plot}, participants produced items with correct initial or final positions at great than chance level only in the segmented condition, but not the continuous condition.
...
```{r recall-positions-plot, fig.height=11, fig.cap="Plot of chunks with correct initial or final positions."}
ggpubr::ggarrange (
...
```{r recall-general-results-print}
...
```{r recall-word-vs-pw-analysis-calculate-city, include = FALSE, eval = FALSE}
